...
std::shared_ptr<execution::server::Server> server;
std::unique_ptr<execution::logger::Logger> logger;
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        auto configManager = std::make_shared<execution::config::ConfigManager>(configPath);
        
        // Инициализация логгера
        logger = std::make_unique<execution::logger::Logger>(
            configManager->getLogLevel(),
            configManager->getLogFile()
        );
        
        logger->info("Сервис управления исполнением работ запускается...");
        logger->info("Загружена конфигурация из: " + configPath);
        
        // Инициализация сервера
        server = std::make_shared<execution::server::Server>(
            configManager,
            logger
        );
        ...
        logger->info("Сервис управления исполнением работ запущен на порту " + 
                    std::to_string(configManager->getServerPort()));
        
        server->waitForTermination();
        
        logger->info("Сервис управления исполнением работ завершил работу");
        return 0;
    } 
